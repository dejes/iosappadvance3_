...
class OthersPostsTableViewController: UITableViewController {
    ...
    func GetSelfpage(userid:String) -> AnyPublisher<SelfPage,Error> {
        let ProfileURL = URL(string: "https://dev-108380.okta.com/api/v1/users/" + userid )
          ...
          override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
           ...
           downloadimage(url: URL(string: userPost[indexPath.row].pic2!)!, imageview: cell.PostImage2b)
            ...
            cancellable = GetSelfpage(userid: userPost[indexPath.row].userid!).receive(on:	 DispatchQueue.main).sink(receiveCompletion: { (_) in
                   
               }, receiveValue: { [weak self](selfdetail) in
                   print(selfdetail)
                   cell.userNameLabel2.text=selfdetail.profile.nickName
                self!.writer.append(selfdetail.profile.nickName!)
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "ToSearch" {
            ...
            let detailController = segue.destination as? SearchViewController
            
            detailController?.uniwriter=writer
        }
    }
